...
public class Controller {
	...
	public void tick()
	{
		...
		if(c.getHealth()<=0)
		{
			...
			if(c.getAnimator().getCurrentAnimation()==c.animDie)
			{
				if(c.animDie.getCycle()==1)
				{
					...
					handler.getWorld().getEntityManager().addEntity(new Loot(handler,c.getX()+c.getBounds().x-32,c.getY()+c.getBounds().y-32));
				}
			}
			else
			{
		...
		if(c.getAnimator().isFixed()){return;}
		...
		if(isX())
		{
			if(c.getMana()<=0){setX(false);}
			else{c.setMana(c.getMana()-20);}
		}
	...
	{
		if(c.getAnimator().getCurrentAnimation()==c.animBowUp ||
				c.getAnimator().getCurrentAnimation()==c.animBowDown ||
				c.getAnimator().getCurrentAnimation()==c.animBowLeft ||
				c.getAnimator().getCurrentAnimation()==c.animBowRight)
				{
		...
		else if(c.getAnimator().getCurrentAnimation()==c.animMagicUp ||
				c.getAnimator().getCurrentAnimation()==c.animMagicDown ||
				c.getAnimator().getCurrentAnimation()==c.animMagicLeft ||
				c.getAnimator().getCurrentAnimation()==c.animMagicRight)
				{
	...
	public void calculateHit(int damage)
	{
		...
		if(c.getFace()==1)
		{
			...
			r = new Rectangle((int)(c.getX()+c.getBounds().x),
						      (int)(c.getY()+c.getBounds().y-c.getHitH()),
					           c.getHitW(),c.getHitH());
		}
		else if(c.getFace()==2)
		{
		...
		else if(c.getFace()==3)
		{
			...
			r = new Rectangle((int)(c.getX()+c.getBounds().x),
				      (int)(c.getY()+c.getBounds().y+c.getBounds().height),
			           c.getHitW(),c.getHitH());
		}
		else
		{
			...
			r = new Rectangle((int)(c.getX()+c.getBounds().x-c.getHitW()),
				      (int)(c.getY()+c.getBounds().y),
			           c.getHitW(),c.getHitH());
		}
	...
	public int canHit(Creature t)
	{
		...
		r = new Rectangle((int)(c.getX()+c.getBounds().x),
					      (int)(c.getY()+c.getBounds().y-c.getBounds().height*3),
